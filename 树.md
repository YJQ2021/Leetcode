...
* 自己的解法<br>
有问题，但是没找出来是哪里有问题。失败的测试用例非常长。<br>
思路：<br>
...
首先findLength(TreeNode root)可以理解为从包含root结点的单向最长同值路径长度。单向是说这条路径要以root为起点，可以向左子树出发，也可以向右子树出发。但是不能以root为拐点。<br>
...
```
三个函数的配合照理来说应该没有什么逻辑问题，但是测试就是通过不了。。。
* 官方解法<br>
官方解法还是秒哇！关键在于:
